//
//  SIUnitLabelConstraintsLandscapeConstraints.swift
...
class SIUnitLabelConstraintsLandscapeConstraints: CalculatorConstraints {
    ...
    func updateCenterXConstraint() {
        guard let openingParenthesisCell: CalculatorCollectionViewCell = self.viewControllerDelegate.scientificCalculatorCollectionView.trackedViewCells[0] else { return }
        
        let siUnitLabel: SIUnitLabel! = viewControllerDelegate.labelView.siUnitLabel
        
        self.centerX = siUnitLabel.centerXAnchor.constraint(equalTo: openingParenthesisCell.centerXAnchor)
    }
}
